...
namespace Engine
{
Mesh::Mesh(nvrhi::DeviceHandle device, nvrhi::CommandListHandle deviceContext, std::vector<Vertex>& Vertices, std::vector<DWORD>& indices, std::vector<Texture> tex, const XMMATRIX& Matrix)
{
	...
	hr = this->indexbuffer.Initialize(device, deviceContext, indices.data(), indices.size());
...
void Mesh::DrawJustMesh(nvrhi::CommandListHandle commandList)
{
	...
	args.
	commandList->draw()
	deviceContext->(0, 1, vertexbuffer.GetAddressOf(), vertexbuffer.StridePtr(), &offset);
	deviceContext->IASetIndexBuffer(indexbuffer.Get(), DXGI_FORMAT_R32_UINT, 0);
	deviceContext->DrawIndexed(indexbuffer.BufferSize(), 0, 0);
}

void Mesh::Draw(nvrhi::DeviceHandle device, nvrhi::CommandListHandle commandList, nvrhi::BindingSetDesc& bindingSetDesc)
{
	UINT offset = 0;
	...
	for (auto& tex : textures)
	{
		...
		case aiTextureType_DIFFUSE:
			bindingSetDesc.bindings.push_back(nvrhi::BindingSetItem::Texture_SRV(0, tex.GetRawTexture().Get()));
			break;
		case aiTextureType_NORMALS:
			bindingSetDesc.bindings.push_back(nvrhi::BindingSetItem::Texture_SRV(1, tex.GetRawTexture().Get()));
			break;
		case aiTextureType_DIFFUSE_ROUGHNESS:
			bindingSetDesc.bindings.push_back(nvrhi::BindingSetItem::Texture_SRV(2, tex.GetRawTexture().Get()));
			break;
		default:
