...
void Application::OnCreate()
{
	...
	HRESULT hr = this->constantBuffer.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = this->floorConstantBuffer.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");


	hr = this->lightConstantBuffer.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = this->CameraInfoConstantBuffer.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_lightparams.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_PrefilteringParams.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_LightSpace.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_ObjectModel.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_ViewProj.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_DebugColors.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");

	hr = m_CastLight.Initialize(gfx.GetDevice(), gfx.m_CommandList);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize constant buffer.");
	...
	if (!helmet.Initialize("Assets/DamagedHelmet/gLTF/DamagedHelmet.gltf", gfx.GetDevice(), gfx.m_CommandList, this->constantBuffer))
		return;
...
//{}
	...
	if (!floor.Initialize(gfx.physicsController.CreateAndAddObject(Floor_settings, EActivation::DontActivate),
		...
		gfx.m_CommandList, this->constantBuffer))
	{
		return;
	}
	...
	hr = m_FullScreenVertex.Initialize(gfx.GetDevice(),gfx.m_CommandList, vertices.data(), vertices.size());
	COM_ERROR_IF_FAILED(hr, "Failed to initialize Vertex buffer.");
	DWORD indices[] = {
	0, 1, 2, // First triangle
	2, 1, 3  // Second triangle
	};
	hr = m_FullScreenIndex.Initialize(gfx.GetDevice(),gfx.m_CommandList, indices, 6);
	...
	hr = m_HdriVertex.Initialize(gfx.GetDevice(),gfx.m_CommandList, cubevertices.data(), cubevertices.size());
	COM_ERROR_IF_FAILED(hr, "Failed to initialize Vertex buffer.");
	DWORD cubeindices[] = {
		// Front face
			0, 1, 2,
	...
	hr = m_HdriIndex.Initialize(gfx.GetDevice(), gfx.m_CommandList, cubeindices, 36);
	COM_ERROR_IF_FAILED(hr, "Failed to initialize index buffer.");
	...
	texDesc.mipLevels = 1;
	texDesc.arraySize = 1;
	texDesc.format = nvrhi::Format::RGB32_FLOAT; // HDR format
	texDesc.isShaderResource = true;

	gfx.HDRITexture = gfx.m_NvrhiDevice->createTexture(texDesc);
	gfx.m_CommandList->writeTexture(gfx.HDRITexture, 0, 0, data, width * sizeof(float) * 3);
...
void Application::InitializeShaders()
{
	...
	std::vector<nvrhi::VertexAttributeDesc> layout = {
	
	{ .name="POSITION",.format = nvrhi::Format::RGB32_FLOAT,  .offset = offsetof(Vertex, pos), .elementStride =sizeof(Vertex) },
	{ .name="TEXCOORD",.format = nvrhi::Format::RG32_FLOAT,.offset = offsetof(Vertex, texCoord), .elementStride = sizeof(Vertex)  },
	{ .name="NORMAL",.format = nvrhi::Format::RGB32_FLOAT, .offset = offsetof(Vertex, normal), .elementStride = sizeof(Vertex)  },
	{ .name="TANGENT",.format = nvrhi::Format::RGB32_FLOAT, .offset = offsetof(Vertex, Tangent), .elementStride = sizeof(Vertex)  },
	{ .name="BITANGENT",.format = nvrhi::Format::RGB32_FLOAT,.offset = offsetof(Vertex, BiTangent), .elementStride = sizeof(Vertex)  }
	
	};

	std::vector<nvrhi::VertexAttributeDesc>  FullScreenRectlayout =
	{
	{.name = "POSITION",.format = nvrhi::Format::RGB32_FLOAT,  .offset = 0, .elementStride = sizeof(float) * 5},
	{.name = "TEXCOORD",.format = nvrhi::Format::RG32_FLOAT,.offset = sizeof(float) * 2, .elementStride = sizeof(float) * 5},
	};
	
	std::vector<nvrhi::VertexAttributeDesc> posDesc=
	{
		{.name = "POSITION",.format = nvrhi::Format::RGB32_FLOAT,  .offset = 0, .elementStride = sizeof(float) * 3},
	};
	std::vector<nvrhi::VertexAttributeDesc> ModelPos =
	{
		{.name = "POSITION",.format = nvrhi::Format::RGB32_FLOAT,  .offset = offsetof(Vertex, pos), .elementStride = sizeof(float) * 3 },
	};

	std::vector<nvrhi::VertexAttributeDesc> Surfelvb =
	{
		{.name = "POSITION",.format = nvrhi::Format::RGB32_FLOAT, .offset = offsetof(SurfelVB, pos),.elementStride = sizeof(SurfelVB) },
		{ .name = "NORMAL",.format = nvrhi::Format::RGB32_FLOAT,.offset = offsetof(SurfelVB, norm), .elementStride = sizeof(SurfelVB) },
		{ .name = "COLOR",.format = nvrhi::Format::RGBA32_FLOAT,.offset = offsetof(SurfelVB, color),.elementStride = sizeof(SurfelVB) },
		{ .name = "RADIUS",.format = nvrhi::Format::R32_FLOAT, .offset = offsetof(SurfelVB, radius),.elementStride = sizeof(SurfelVB) }
	};

	if (!m_vertexShader.Initialize(gfx.GetDevice(), L"CompiledShaders/VertexSshader_v.cso", layout, layout.size()))
	{
		return;
	}	
	
	if (!m_pixelShader.Initialize(gfx.GetDevice(), L"CompiledShaders/PixelShader_p.cso"))
	{
		return;
	}

	if (!m_GBuffervertexShader.Initialize(gfx.GetDevice(), L"CompiledShaders/GBufferVert_v.cso", layout, layout.size()))
	{

		return;
	}

	if (!m_GBufferpixelShader.Initialize(gfx.GetDevice(), L"CompiledShaders/GBufferPixel_p.cso"))
	{
		return;
	}

	if (!m_DeferredvertexShader.Initialize(gfx.GetDevice(), L"CompiledShaders/DefferedVert_v.cso", FullScreenRectlayout, FullScreenRectlayout.size()))
	{

		return;
	}

	if (!m_DeferredpixelShader.Initialize(gfx.GetDevice(), L"CompiledShaders/DefferedPixel_p.cso"))
	{
		return;
	}
	if (!m_EquiToHDRI_VS.Initialize(gfx.GetDevice(), L"CompiledShaders/EquiToHdri_v.cso", posDesc, posDesc.size()))
	{
		return;
	}
	if (!m_EquiToHdri_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/EquiToHdri_p.cso"))
	{
		return;
	}

	if (!m_IrradianceConvolution_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/IrradianceConvolution_p.cso"))
	{
		return;
	}
	if (!m_BackgroundCubemap_VS.Initialize(gfx.GetDevice(), L"CompiledShaders/BackgroundCubemap_v.cso", posDesc, posDesc.size()))
	{
		return;
	}
	if (!m_BackgroundCubemap_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/BackgroundCubemap_p.cso"))
	{
		return;
	}
	if (!m_Prefiltering_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/Prefiltering_p.cso"))
	{
		return;
	}
	if (!m_BRDF_VS.Initialize(gfx.GetDevice(), L"CompiledShaders/BRDF_v.cso", FullScreenRectlayout, FullScreenRectlayout.size()))
	{
		return;
	}	
	if (!m_BRDF_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/BRDF_p.cso"))
	{
		return;
	}
	if (!m_ShadowDepth_VS.Initialize(gfx.GetDevice(), L"CompiledShaders/ShadowMapDepth_v.cso", ModelPos, ModelPos.size()))
	{
		return;
	}
	if (!m_ShadowDepth_GS.Initialize(gfx.GetDevice(), L"CompiledShaders/ShadowMapDepth_g.cso"))
	{
		return;
	}
	if (!m_DebugCascade_VS.Initialize(gfx.GetDevice(), L"CompiledShaders/DebugCascade_v.cso", ModelPos,ModelPos.size()))
	{
		return;
	}
	if (!m_DebugCascade_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/DebugCascade_p.cso"))
	{
		return;
	}
	if (!m_DebugDrawShadowMap_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/DrawShadowMap_p.cso"))
	{
		return;
	}

	if (!m_SurfelDebug_VS.Initialize(gfx.GetDevice(), L"CompiledShaders/SurfelDebug_v.cso", Surfelvb, Surfelvb.size()))
	{
		return;
	}
	if (!m_SurfelDebug_PS.Initialize(gfx.GetDevice(), L"CompiledShaders/SurfelDebug_p.cso"))
	{
		return;
	}
	if (!m_SureflDebug_GS.Initialize(gfx.GetDevice(), L"CompiledShaders/SurfelDebug_g.cso"))
	{
		return;
	}
	//GBuffer
	{
		...
		m_GBufferPipeline = gfx.GetDevice()->createGraphicsPipeline(pipelineDesc, gfx.GBUfferFrameBuffer);
	}
	// Lighting Pass
	{
		nvrhi::BindingLayoutDesc layoutDesc;
...
void Application::RenderFrame()
{
	...
	ImGui::Image((ImTextureID)gfx.positionTexture.Get(), {25,25 });
	ImGui::Image((ImTextureID)gfx.NormalTexture.Get(), { 25,25 });
	ImGui::Image((ImTextureID)gfx.DiffuseTexture.Get(), { 25,25 });
	ImGui::Image((ImTextureID)gfx.SpecularTexture.Get(), { 25,25 });
	...
	gfx.m_CommandList->SetDescriptorHeaps(1, gfx.ImguiHeap);

	ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), (ID3D12GraphicsCommandList*)gfx.m_CommandList->getNativeObject(nvrhi::ObjectTypes::D3D12_GraphicsCommandList).pointer);
